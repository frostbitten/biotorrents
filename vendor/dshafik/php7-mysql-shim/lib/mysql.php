...
namespace {
    ...
    if (!function_exists('\mysql_connect')) {
        ...
        function mysql_fetch_assoc($result) /* : array|null */
        {
			...
			try { 
				if(is_bool($result)) throw new \Exception("BOOL given instead of result");
			} catch (\Exception $e){
        ...
        public static function checkValidResult($result, $function)
        {
            if (!($result instanceof \mysqli_result)) {
                if ($function != "mysql_fetch_object") {
					...
					try { 
						throw new \Exception($function . "() expects parameter 1 to be resource, " . strtolower(gettype($result)) . " given");
					} catch (\Exception $e){
						error_log($e->getTraceAsString());
					}
                }
